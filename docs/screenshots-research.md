...
Both were generated by running the harness in this environment against the committed
fixture vault — the real `IndexPanel`, real Obsidian 1.13.4, no hand-editing. The
prototype forces Obsidian's **dark base theme** before shooting, so every screenshot is
dark (see `before()` in `capture.e2e.ts`).
